...
public class MeanItemBasedItemRecommender extends AbstractItemBasedItemRecommender {
     ...
     * </p>
     *
     * @param m The model containing item mean ratings. LensKit will automatically
     *          build an {@link ItemMeanModel}
     *          object. Its use as a parameter type in this constructor declares it
     *          as a <em>dependency</em> of the
     ...
     * {@inheritDoc}
     *
     * This is the LensKit recommend method. It takes several parameters; we
     * implement it for you in terms of a
     ...
     *
     * <p>
     * Your code needs to obtain the mean rating, if one is available, for each
     * item, and return a list of the
     * {@code n} highest-rated items, in decreasing order of score.
     * </p>
     *
     * <p>
     * To create the {@link ResultMap} data structure, do the following:
     * </p>
     *
     * <ol>
     * <li>Create a {@link List} to hold {@link Result} objects.</li>
     * <li>Create a result object for each item that can be scored. Use
     * {@link Results#create(long, double)} to
     * create the result object. If an item cannot be scored (because there is no
     * mean available), ignore it and
     * do not add a result to the list.</li>
     * <li>Convert the list of results to a {@link ResultList} using
     * {@link Results#newResultList(List)}.</li>
     * </ol>
     *
     * @param n     The number of items to recommend. If this is negative, then
     *              recommend all possible items.
    ...
    private ResultList recommendItems(int n, LongSet items) {
        ...
        LongIterator iterator = items.iterator();
        while (iterator.hasNext()) {
            long key = iterator.next();
            double meanScore = model.getMeanRating(key);
            if (meanScore > -1) {
                Result result = Results.create(key, meanScore);
                results.add(result);
            }
        }
