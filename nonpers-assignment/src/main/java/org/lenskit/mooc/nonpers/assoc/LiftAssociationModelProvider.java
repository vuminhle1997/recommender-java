...
public class LiftAssociationModelProvider implements Provider<AssociationModel> {
    ...
    public AssociationModel get() {
        ...
        for (Long2ObjectMap.Entry<LongSortedSet> xEntry : itemUsers.long2ObjectEntrySet()) {
            ...
            // TODO Compute lift association formulas for all other 'Y' items with respect
            // to this 'X'
            // loop over the 'y' items
            // j is the reference, X
            // i is the estimated score for the reference, so Y
            int count = 0;
            for (Long2ObjectMap.Entry<LongSortedSet> yEntry : itemUsers.long2ObjectEntrySet()) {
                ...
                for (long yUserId : yUsers) {
                    if (xUsers.contains(yUserId)) {
                        count++;
                    }
                }
                double numeratorFraction = (float) count * allUsers.size();
                double denominatorFraction = (float) xUsers.size() * yUsers.size();
                double probability = numeratorFraction / denominatorFraction;
