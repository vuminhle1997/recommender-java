...
public class SimpleItemItemModelProvider implements Provider<SimpleItemItemModel> {
    ...
    public SimpleItemItemModel get() {
        ...
        for (Map.Entry<Long, Long2DoubleMap> i : itemVectors.entrySet()) {
            Long2DoubleMap similarities = new Long2DoubleOpenHashMap();
            for (Map.Entry<Long, Long2DoubleMap> j : itemVectors.entrySet()) {
                if (i.getKey() != j.getKey()) {
                    Double numerator = Vectors.dotProduct(i.getValue(), j.getValue());
                    Double denominator = Vectors.euclideanNorm(i.getValue()) * Vectors.euclideanNorm(j.getValue());
                    Double cosineSimlarity = numerator / denominator;
                    if (cosineSimlarity > 0)
                        similarities.put(j.getKey(), cosineSimlarity);
                }
            }
            itemSimilarities.put(i.getKey(), similarities);
        }
