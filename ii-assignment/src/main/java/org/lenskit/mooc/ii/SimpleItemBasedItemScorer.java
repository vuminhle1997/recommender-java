...
public class SimpleItemBasedItemScorer extends AbstractItemBasedItemScorer {
     ...
     * @param items  The score vector. Its domain is the items to be scored, and the
     *               scores should
    ...
    public ResultMap scoreRelatedItemsWithDetails(@Nonnull Collection<Long> basket, Collection<Long> items) {
        ...
        for (long item : items) {
            double score = 0;
            Long2DoubleMap similarities = this.model.getNeighbors(item);
            for (long basketItem : basket) {
                score += similarities.get(basketItem);
            }
